...
const APP = {
  name: "Base Horizon View (Built for Base)",
...
async function readNetworkStats(address: string) {
  const c = client();
...
function mount() {
  ...
  subtitle.textContent =
    "Wallet connection, Base chain monitoring, and real-time blockchain stats view.";
  ...
  btnConnect.onclick = async () => {
    try {
      ...
      const stats = await readNetworkStats(session.address);
